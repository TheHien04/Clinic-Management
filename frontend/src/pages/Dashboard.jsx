...
function StatusPieChart({ data }) {
  return (
    <div className="chart-widget">
      <div className="chart-widget-title">Appointment Status Ratio</div>
...
function KPIBarChart({ data }) {
  return (
    <div className="chart-widget">
      <div className="chart-widget-title">Appointments in Last 7 Days</div>
      <ResponsiveContainer width="100%" height={220}>
    ...
    <div className="dashboard-layout">
      ...
      <div className="dashboard-main">
        ...
        <main className="dashboard-content">
          <h2>Welcome to Clinic Management Dashboard</h2>
          
          {/* KPI tổng hợp */}
          <div className="dashboard-stats-row">
            <div className="stats-container">
              <div className="stat-card">
                <div className="stat-title"><FaCalendarAlt style={{color:'#1976d2'}}/>Today's Appointments</div>
                <div className="stat-value">{kpi.today}</div>
              </div>
              <div className="stat-card">
                <div className="stat-title"><FaUser style={{color:'#fbc02d'}}/>Waiting</div>
                <div className="stat-value">{kpi.waiting}</div>
              </div>
              <div className="stat-card">
                <div className="stat-title"><FaChartBar style={{color:'#43a047'}}/>Completed</div>
                <div className="stat-value">{kpi.completed}</div>
              </div>
              <div className="stat-card">
                <div className="stat-title"><FaUserMd style={{color:'#1976d2'}}/>Doctors</div>
                <div className="stat-value">{doctors.length}</div>
              </div>
              <div className="stat-card">
                <div className="stat-title"><FaUser style={{color:'#1976d2'}}/>Patients</div>
          ...
          <div style={{display:'flex', gap:24, justifyContent:'center', margin:'24px 0', flexWrap:'wrap'}}>
            <button onClick={()=>handleShortcut('book')} style={{background:'#1976d2',color:'#fff',border:'none',borderRadius:8,padding:'12px 28px',fontWeight:600,fontSize:'1.08rem',cursor:'pointer',display:'flex',alignItems:'center',gap:8}}>
              <FaCalendarAlt/> Book New Appointment
            </button>
            <button onClick={()=>handleShortcut('schedule')} style={{background:'#fff',color:'#1976d2',border:'1.5px solid #1976d2',borderRadius:8,padding:'12px 28px',fontWeight:600,fontSize:'1.08rem',cursor:'pointer',display:'flex',alignItems:'center',gap:8}}>
              <FaUserMd/> Doctor Schedules
            </button>
            <button onClick={()=>handleShortcut('kpi')} style={{background:'#fff',color:'#1976d2',border:'1.5px solid #1976d2',borderRadius:8,padding:'12px 28px',fontWeight:600,fontSize:'1.08rem',cursor:'pointer',display:'flex',alignItems:'center',gap:8}}>
